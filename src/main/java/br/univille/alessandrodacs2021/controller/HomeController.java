package br.univille.alessandrodacs2021.controller;
import java.text.SimpleDateFormat;
import java.util.Date;
import java.util.HashMap;
import java.util.Locale;

import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RequestMapping;
//import org.springframework.web.bind.annotation.ResponseBody;
import org.springframework.web.servlet.ModelAndView;

@Controller
@RequestMapping("/")
public class HomeController {
    
    @GetMapping
    public ModelAndView index()
    {
        //String  sql= "Select * from produto" exemplo de acoplagem (errado,,  necessário gatway)

        
        Locale.setDefault(Locale.US);
        Date dataAgora = new Date();
        SimpleDateFormat sdf 
        =new SimpleDateFormat("dd/MM/yyyy  hh:mm:ss");
        
        HashMap<String,String> dados =new HashMap<>();
        dados.put("tempo",sdf.format(dataAgora));
        dados.put("nomeapp","Programando Site?");
        dados.put("produtor", "Alessandro");
        dados.put("padrao", "M.V.C");
        
        return new ModelAndView("home/index",dados);
        //return new ModelAndView("home/index","tempo",sdf.format(dataAgora));
        //return new ModelAndView("home/index","tempo",dataAgora.toString());
    }
    

    /*
    @RequestMapping("/outro")
    @ResponseBody
    public String outraPagina()
    {
        return "Outra pagina";
    }*/
}
